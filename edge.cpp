...
bool Edge::singlePointIntersect(const Edge& e2) const
{
  ...
  if (test1 == 0 && test2 == 0)
  {
    // Edges are collinear. Check for endpoint touching only.
    if ((v == e2.u && u != e2.v) || (u == e2.v && v != e2.u) ||
        (v == e2.v && u != e2.u) || (u == e2.u && v != e2.v))
      return true;
...
bool Edge::overlap(const Edge& e2) const
{
  ...
  {
    bool xOverlap = std::max(u.x, v.x) >= std::min(e2.u.x, e2.v.x) &&
      std::min(u.x, v.x) <= std::max(e2.u.x, e2.v.x);
    bool yOverlap = std::max(u.y, v.y) >= std::min(e2.u.y, e2.v.y) &&
      std::min(u.y, v.y) <= std::max(e2.u.y, e2.v.y);
